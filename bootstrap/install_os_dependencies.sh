...
# This script is part of the COOKIECUTTER Django Project
# https://github.com/pydanny/cookiecutter-django

WORK_DIR="$(dirname "$0")"
PROJECT_DIR="$(dirname "$WORK_DIR")"
DISTRO_NAME=$(lsb_release -sc)
OS_REQUIREMENTS_FILENAME="../requirements/so-requirements-$DISTRO_NAME.apt"
...
if [ ! -r "$OS_REQUIREMENTS_FILENAME" ]; then
    cat <<-EOF >&2
		...
		There is no requirements file for your distribution ($OS_REQUIREMENTS_FILENAME)
...
function list_packages(){
    grep -v "#" "../requirements/${OS_REQUIREMENTS_FILENAME}" | grep -v "^$";
}
